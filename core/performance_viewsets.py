...
class SubjectPerformanceViewSet(viewsets.ViewSet):
    ...
    def summary(self, request):
        ...
        # Query all results ONCE - don't re-query inside the loop
        ...
        attended_count = all_session_attendances.filter(
            status__in=['present', 'late']
        ).count()
        expected_attendances = total_students * total_sessions
        overall_attendance_rate = (attended_count / expected_attendances * 100) if expected_attendances > 0 else 0
        ...
        for enrollment in enrolled_students:
            ...
            # Filter the ALREADY QUERIED all_results for this specific student
        ...
        for session in attendance_sessions:
            ...
            present_count = session_attendances.filter(status='present').count()
            late_count = session_attendances.filter(status='late').count()
            ...
            session_breakdown.append({
                ...
                'attendance_rate': round((attended / total_students * 100), 2) if total_students > 0 else 0
            })
        ...
        return Response({
            ...
            'overall_statistics': {
                ...
                'total_results_submitted': all_results.count() if all_results.exists() else 0,
                ...
                'actual_attendances': attended_count,
...
class ClassPerformanceViewSet(viewsets.ViewSet):
    ...
    def summary(self, request):
        ...
        attendance_sessions = AttendanceSession.objects.filter(class_obj=class_obj)
        total_sessions = attendance_sessions.count()
        ...
        for enrollment in enrollments:
            ...
            all_student_attendances = SessionAttendance.objects.filter(
                session__class_obj=class_obj,
                student=student
            )
            present_count = all_student_attendances.filter(status='present').count()
            late_count = all_student_attendances.filter(status='late').count()
            ...
            for subject in subjects:
                ...
                if subject_results.exists():
                    ...
                    subject_attendances = SessionAttendance.objects.filter(
                        session__subject=subject,
                        student=student,
                        status__in=['present', 'late']
                    )
        ...
        for subject in subjects:
            ...
            subj_attendances = SessionAttendance.objects.filter(
                session__subject=subject,
                status__in=['present', 'late']
            )
    ...
    def top_performers(self, request):
        ...
        for enrollment in enrollments:
            ...
            if student_results.exists():
                ...
                exam_percentage = (total / possible * 100) if possible > 0 else 0
            else:
                exam_percentage = 0
        ...
        # Sort by combined score instead of just exam percentage
        student_data.sort(key=lambda x: x['combined_score'], reverse=True)
