...
class ZKTecoSyncService:
    ...
    def connect(self):
        try:
            ...
            logger.error(
                f'Failed to connect to {self.device.name}: {e}'
            )

            self.device.last_sync_status = (
                f'connection_failed: {str(e)}'
            )[:50]

            self.device.save(
                update_fields=['last_sync_status']
            )
    ...
    def _process_single_log(self, log):
        ...
        # Use device timestamp directly
        device_tz = pytz.timezone(
            'Africa/Nairobi'
        )

        scan_time = device_tz.localize(
            log.timestamp
        )
    ...
    def _resolve_student(
        ...
        student = User.objects.filter(
            ...
            role='student',
            is_active=True,
            school_memberships__school=(
                self.device.school
            ),
            school_memberships__status=(
                'active'
            )
        ).first()
    ...
    def _update_sync_status(
        ...
        self.device.last_sync_records = (
            count
        )

        self.device.total_synced_records += (
            count
        )
