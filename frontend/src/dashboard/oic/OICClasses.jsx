...
import { getOICClasses } from '../../lib/api'
...
export default function OICClasses() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await getOICClasses(params)
    ...
    <div className="p-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4">
        <div className="flex-1 min-w-0">
          <h2 className="text-lg sm:text-xl font-semibold text-black">My Assigned Classes</h2>
          <p className="text-xs sm:text-sm text-neutral-500 mt-1">Classes you are assigned to as OIC. Click to view details.</p>
        </div>
        <input
          type="text"
          placeholder="Search..."
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {loading ? (
          ...
          <div className="col-span-full text-sm text-neutral-400">No assigned classes found.</div>
        ) : (
          classes.map((cls) => (
            <div key={cls.id}>
              <Card
